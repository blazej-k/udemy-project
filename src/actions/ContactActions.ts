...
export const CONTACT_SENDREQUEST = 'contact_sendrequest'
...
export const getMessages = () => async (dispatch: Dispatch<ContactReducerType>) => {
    ...
    try {
        ...
        dispatch({
            type: MESSAGESERROR, payload: "Upss, something go wrong and we can't get messages! Please go back \n" +
                "some time later!"
        })
    }
}
...
export const sendMessage = (message: string) => async (dispatch: Dispatch<ContactReducerType>) => {
    ...
    try {
        ...
        dispatch({
            type: MESSAGESERROR, payload: "Upss, something go wrong and we can't send message! Please go back \n" +
                "some time later!"
        })
    }
}